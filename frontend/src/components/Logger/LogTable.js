...
function LogTable() {
    ...
    useEffect(() => {
        ...
        const startProcess = async () => {
            try {
                const response = await axios.post('http://localhost:8000/logger/load_logs/');
                ...
                if (response.status === 200) {
                    ...
                    const eventSource = new EventSource('http://localhost:8000/logger/stream_events/');
                    ...
                    eventSource.onmessage = function(event) {
                        ...
                        setTableData(prevData => sortDataByTimestamp([...prevData, ...data]));
  ...
  return (
    <div className='m-5'>
        ...
        <DataTable columns={columns} data={filteredData} onRowClicked={handleRowClick} customStyles={customStyles}  />


    <Offcanvas show={showOffcanvas} onHide={handleOffcanvasToggle}   className="custom-offcanvas" placement="top">
